...
const validOperators = ['eq', 'ne', 'gt', 'gte', 'lt', 'lte', 'in', 'nin', 'all', 'exists',
  'eqa',
...
const defaultDataTypeConverters = {
  ...
  int(str) { const i = parseInt(str, 10); return isNaN(i) ? undefined : i; },
  float(str) { const i = parseFloat(str); return isNaN(i) ? undefined : i; },
  date(str) { const d = new Date(str); return isNaN(d.getTime()) ? undefined : d; },
  bool(str) {
...
module.exports = function qpm(opts) {
  ...
  for (let i = 0; i < defaultAutoDetectTypes.length; i += 1) {
...
|| (ad.fieldPattern && ad.fieldPattern.test(paramName))) {
    ...
    */
    ...
    const sortSpecs = typeof params.__sort === 'string' ? params.__sort.split(',') : params.__sort;
    sortSpecs.forEach((s) => {
      let direction = 1;
      let sortField = s;
