...
exports.getAnswer = handler.getOne(Answer, {select: { answer: 1}});
...
exports.checkCanView = catchAsync(async (request, response, next) => {
  ...
  // answer of question which user is owner
  if (request.user.id == answer.question.user.id && answer.question.status == 'closed') return next(); 
...
exports.checkCanUpdate = catchAsync(async (request, response, next) => {
  ...
  if (answer.question.status == 'closed')
    return next(new AppError('This question of Answer is closed, You dont have permission to do this', StatusCodes.FORBIDDEN ));
  // answer owner
  if (request.user.id == answer.user.id ) return next(); 
...
exports.setQuestionUserIds = (request, response, next) => {
  ...
  if (!request.body.question && request.params.questionId) request.body.question = request.params.questionId;
